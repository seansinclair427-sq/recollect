...
只监听 127.0.0.1。凡是有副作用的接口（扫描、打开文件、改配置）都要带
启动时生成的一次性 token —— 否则你浏览器里随便一个网页都能对
    ...
    def _api_get(self, ep: str):
        ...
        if ep == "projects":
            ...
            for r in rows:
                ...
                d["third_party"] = projmod._third_party(d["path"])
                out.append(d)
            # 你亲手做的排前面，解压/安装出来的沉底
        ...
        if ep == "scan/status":
            return self._json(_scan_progress.as_dict())

        if ep == "config":
            c = _cfg()
            return self._json({"roots": c.roots, "port": c.port,
                               "max_text_bytes": c.max_text_bytes,
    ...
    def _api_post(self, ep: str, body: dict):
        ...
        if ep == "config":
            ...
            if isinstance(roots, list):
                ...
                c.roots = good
            if isinstance(body.get("index_content"), bool):
                c.index_content = body["index_content"]
            c.save()
            return self._json({"ok": True, "roots": c.roots})
    ...
    def _start_scan(self, full: bool):
        ...
        with _scan_lock:
            ...
            def work():
                con = store.connect()
                try:
                    scan.scan(con, _cfg(), _scan_progress, full=full)
                except Exception as exc:
                    _scan_progress.phase = "error"
                    _scan_progress.message = "%s: %s" % (type(exc).__name__, exc)
                    _scan_progress.finished = time.time()
                finally:
                    con.close()
...
def _is_exported(path: str) -> bool:
    """导出的年鉴刚生成、还没进索引，也要允许打开。"""
    base = os.path.basename(path)
    return base.endswith("年作品年鉴.html") and os.path.isfile(path)


# ----------------------------------------------------------------- 启动
def _bind(port: int, tries: int = 12):
    """端口被占了就往后挪。双击启动的人不该看见 traceback。"""
    last = None
...
def serve(port: int = 7331, open_browser: bool = True) -> None:
    ...
    if open_browser:
        threading.Timer(0.6, lambda: _launch(url)).start()
    try:
        httpd.serve_forever()
